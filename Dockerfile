...
COPY ./sources/gmp-6.2.1.tar.xz /home/sage/
RUN cd /home/sage/ && \
...
COPY ./sources/mpfr-4.2.1.tar.gz /home/sage/
RUN cd /home/sage/ && \
...
COPY ./sources/flint-3.1.3.tar.gz /home/sage/
RUN cd /home/sage/ && \
    tar -xvzf flint-3.1.3.tar.gz && \
    rm flint-3.1.3.tar.gz && \
    cd flint-3.1.3/ && \
    ./bootstrap.sh && \
    ./configure && \
    make -j$(nproc) && \
...
COPY ./sources/msolve-0.7.3.tar.gz /home/sage/
RUN cd /home/sage/ && \
    tar -xvzf msolve-0.7.3.tar.gz && \
    rm msolve-0.7.3.tar.gz && \
    cd msolve-0.7.3/ && \
    ./autogen.sh && \
    ./configure && \
    make -j$(nproc) && \
